...
app.config.update(
    ...
    DATABASE=Path(app.instance_path) / 'users_v9.db',
...
# Keep the dataframe row order identical to rebuild_skin_tfidf.load_training_data().
...
def index():
    ...
    return render_template(
        'index_9.html', recent_history=recent_history,
...
def register():
    ...
    if request.method == 'POST':
        ...
        flash(error, 'error')
    return render_template('auth_9.html', mode='register')
...
def login():
    ...
    return render_template('auth_9.html', mode='login')
...
def admin_users():
    ...
    return render_template(
        'admin_9.html', users=users, stats=stats,
...
def recommend_by_review(user_input, gender, age, skin_type):
    ...
    similarities = linear_kernel(
        tfidf_reviews.transform([cleaned]),
...
def build_response(row, user_input, gender, age, skin_type, cleaned=''):
    ...
    return jsonify({
        ...
        'analysis_basis': build_analysis_basis(
            row, user_input, cleaned, gender, age, skin_type,
        ),
