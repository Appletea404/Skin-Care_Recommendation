...
```bash
.venv/bin/python web_app_v9.py
```

기본 주소는 `http://127.0.0.1:5006`입니다. 운영 환경에서는 관리자 계정과
세션 키를 환경 변수로 지정하세요.
...
- `SKINSCOPE_PORT`: 기본값 `5006`
- `SKINSCOPE_DEBUG`: `1`, `true`, `yes` 중 하나일 때만 디버그 모드 활성화

## 테스트

```bash
.venv/bin/python -m unittest -v test_web_app_v9.py
```
...
v9 전문가 분석은 `web_app_v4_rebuild.py`와 같은 핵심 파이프라인을 사용합니다.
...
5. 전체 상담 행의 cosine similarity에 성별·연령대·피부 타입 일치 항목마다 0.05의 프로필 보너스를 합산
